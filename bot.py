        ...
        if message:
            try:
                if "Progress:" in message.text:
                    await message.edit_text(
                        f"\u2705 Search session ended. Total NIDs checked: `{checked_nid_counts.get(chat_id, 0)}`.",
                        parse_mode=constants.ParseMode.MARKDOWN_V2
                    )
            except Exception as e:
...
async def search_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in ongoing_searches and not ongoing_searches[chat_id].done():
        await update.message.reply_text("\u231b You already have an active search running. Please `/cancel` it first.", parse_mode=constants.ParseMode.MARKDOWN_V2)
        return
    ...
    try:
        ...
        if end_nid - start_nid + 1 > 50000000:
            await update.message.reply_text("NID range too large. Limit to 50,000,000 at a time.", parse_mode=constants.ParseMode.MARKDOWN_V2)
            return
        ...
        await update.message.reply_text("Please provide valid numerical NID values.", parse_mode=constants.ParseMode.MARKDOWN_V2)
    except Exception as e:
        logger.error(f"Error in search_command: {e}", exc_info=True)
        await update.message.reply_text(f"Unexpected error: `{escape_markdown_v2(str(e))}`", parse_mode=constants.ParseMode.MARKDOWN_V2)
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in ongoing_searches and not ongoing_searches[chat_id].done():
        ...
        await update.message.reply_text("Cancelling your ongoing NID search...", parse_mode=constants.ParseMode.MARKDOWN_V2)
    else:
        await update.message.reply_text("No active NID search found.", parse_mode=constants.ParseMode.MARKDOWN_V2)
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in ongoing_searches and not ongoing_searches[chat_id].done():
        ...
        await update.message.reply_text(
            f"\ud83d\udd0d An NID search is active.\nChecked: `{checked_count}` / `{total_count}`\nCurrently on NID: `{current_nid}`",
            parse_mode=constants.ParseMode.MARKDOWN_V2
        )
    else:
        await update.message.reply_text("No active NID search.", parse_mode=constants.ParseMode.MARKDOWN_V2)
