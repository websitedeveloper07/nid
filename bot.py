...
# === CONFIG ===
TOKEN = os.getenv("BOT_TOKEN", "8337213161:AAGC0grEHd4MSZS2IfzPFIuyQ1fohUsy3vc")
OWNER_ID = int(os.getenv("OWNER_ID", "8493360284"))
...
logging.basicConfig(
    ...
    level=logging.DEBUG  # switched to DEBUG for visibility
)
...
async def search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if start_nid <= 0 or end_nid <= 0:
            await safe_send(update.message.reply_text, "⚠️ NIDs must be positive integers.")
            return
        if start_nid > end_nid:
            await safe_send(update.message.reply_text, "⚠️ Start NID must be ≤ End NID.")
            return
        if chat_id in ongoing_searches and not ongoing_searches[chat_id].done():
            await safe_send(update.message.reply_text, "⏳ A scan is already running. Use `/cancel` first.")
            return
    ...
    try:
        async with aiohttp.ClientSession() as session:
            for i in range(start_nid, end_nid + 1, batch_size):
                ...
                found_msgs = []
                for result_nid, result_title in results:
        ...
        await safe_send(context.bot.send_message, chat_id=chat_id,
            text=f"❌ Error: `{escape_markdown_v2(str(e))}`")
    finally:
...
def main():
    ...
    logger.info(f"Using TOKEN prefix: {TOKEN[:8]}… Owner ID: {OWNER_ID}")
    ...
    app.add_handler(CommandHandler(["start", "help", "search", "cancel", "status"],
        unauthorized_command, ~owner_filter))

    logger.info(f"🚀 Bot started for Owner ID: {OWNER_ID}")
    try:
        app.run_polling(drop_pending_updates=True)
    except Exception as e:
        logger.error(f"❌ Bot crashed on startup: {e}", exc_info=True)
